...
export default function SetupPage({ initial, error, onSave }: Props) {
  ...
  return (
    <div className="nx-setup">
      <div className="nx-setup__scroll">
        <div className="nx-setup__card">
          <header className="nx-setup__head">
            <div className="nx-setup__logo" aria-hidden="true">N</div>
            <p className="nx-setup__brand">NURAI · DRIVER</p>
            <h1>ربط المركبة</h1>
          ...
          <form id="nx-setup-form" className="nx-setup__form" onSubmit={submit}>
            <label>
              <span>Server URL</span>
              <input value={serverUrl} onChange={(e) => setServerUrl(e.target.value)} required dir="ltr" placeholder="http://host:8080" />
            </label>
            ...
            <label>
              ...
              <input value={vehicleId} onChange={(e) => setVehicleId(e.target.value)} required placeholder="مثال: بغداد-01" />
            </label>
            <label>
              <span>API Key</span>
              <input value={apiKey} onChange={(e) => setApiKey(e.target.value)} type="password" required dir="ltr" autoComplete="off" />
            </label>
      ...
      <footer className="nx-setup__bar">
        {errMsg && <p className="nx-setup__err" role="alert">{errMsg}</p>}
        <button type="submit" form="nx-setup-form" className="nx-setup__submit" disabled={loading}>
          {loading ? 'جاري الاتصال…' : 'بدء القيادة'}
        </button>
      </footer>
    </div>
  );
